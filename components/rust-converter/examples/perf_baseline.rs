...
fn repeat_to_size(seed: &[u8], target_size: usize) -> Vec<u8> {
    ...
    out.extend_from_slice(b"<!DOCTYPE html><html><head><meta charset=\"UTF-8\"><title>bench</title></head><body>\n");
...
fn print_breakdown(sample: &Sample, b: &BreakdownSummary, ffi_avg_ms: f64) {
    ...
    println!("## Stage Breakdown ({}, {})", sample.name, sample.target_label);
    println!();
    ...
    for (name, v) in [
        ...
        let share = if b.total_ms > 0.0 { v / b.total_ms * 100.0 } else { 0.0 };
        println!("| {} | {:.3} | {:.1}% |", name, v, share);
    }
    println!("| direct total | {:.3} | 100.0% |", b.total_ms);
    println!("| ffi end-to-end avg | {:.3} | - |", ffi_avg_ms);
    println!("| inferred ffi/runtime overhead | {:.3} | - |", ffi_overhead);
    println!();
}
...
fn main() {
    ...
    let medium = samples.iter().find(|s| s.name == "medium").expect("medium sample");
