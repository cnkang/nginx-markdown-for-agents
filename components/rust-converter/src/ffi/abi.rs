...
pub const MARKDOWN_HEADER_HASH: u64 = 0xedb7b47db9ed0829;
...
pub const MARKDOWN_SYMBOL_SET_HASH: u64 = 0x0492468d15617cc7;
...
pub struct MarkdownOptions {
    ...
    /// above. The incremental path intentionally ignores `memory_budget`: it
    /// uses only `parser_memory_budget` for parser working state and its fixed
    /// 64 MiB maximum buffered-input cap. This keeps the incremental path's
    /// parser contract explicit instead of implying that it has the
    /// streaming working-set budget model.
