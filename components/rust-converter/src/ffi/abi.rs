...
pub const MARKDOWN_ABI_VERSION: u32 = 3;
...
pub const MARKDOWN_HEADER_HASH: u64 = 0xc2a0db807fd5511d;
...
pub const MARKDOWN_SYMBOL_SET_HASH: u64 = 0x2cdb1bf77b851cc1;
...
pub const DECOMP_CATEGORY_RATIO_EXCEEDED: u32 = 106;

/// Content-Encoding chain parse classification: valid supported chain.
pub const ENCODING_CHAIN_VALID: u8 = 0;
...
// The `NEGOTIATE_WILDCARD_STRICT` / `NEGOTIATE_WILDCARD_ALLOW` mode constants
// were removed in 0.9.2 along with wildcard negotiation: negotiation is now
// strict-only (design §14(g); Requirements LTS-R010, LTS-R023).
...
pub struct FFIBaseUrlInput {
    ...
    pub x_forwarded_for: *const u8,
    /// Length of `x_forwarded_for`.
    pub x_forwarded_for_len: usize,
    ...
    pub x_forwarded_port: *const u8,
    /// Length of `x_forwarded_port`.
    pub x_forwarded_port_len: usize,
...
#[repr(C)]
pub struct FFIEncodingChainResult {
...
#[repr(C)]
pub struct FFIChainDecodeResult {
...
// ─── Error Classification FFI types ─────────────────────────────────

/// FFI-safe error class enum.
///
...
/// during the pre-LTS convergence; `HeaderPlanApplyError` moved 8 → 6 and
/// `StreamingMidFlightError` moved 9 → 7 to keep the set contiguous (8 total).
/// This mirrors the identical renumber of `ErrorClass` in
/// `error/classification.rs`; the cross-enum static asserts below enforce the
/// pairing. Discriminants are additive-only after 1.0.
#[repr(u8)]
...
pub enum FFIErrorClass {
    ...
    /// Worker inflight limit exceeded (inflight).
    Overload = 5,
...
mod layout_tests {
    ...
    /// Post-selector-removal `MarkdownOptions` ABI layout (§14(h),
    /// LTS-R009 / LTS-R023).
    ///
    /// The four custom-selector fields (`prune_selectors`,
    /// `prune_selector_len`, `prune_protection_selectors`,
    /// `prune_protection_selector_len`, formerly at offsets 64/72/80/88)
    /// were removed in task 9.3, shrinking the struct from 128 bytes to 96
    /// bytes and shifting every subsequent retained field down by 32 bytes.
    /// The retained tail fields now land at:
    ///
    /// | field                  | former offset | current offset |
    /// |------------------------|---------------|----------------|
    /// | `memory_budget`        | 96            | 64             |
    /// | `parse_timeout_ms`     | 104           | 72             |
    /// | `parser_memory_budget` | 112           | 80             |
    /// | `flush_threshold`      | 120           | 88             |
    ///
    /// The retained head fields (`flavor` .. `prune_noise`, offsets 0..56)
    /// are unaffected by the removal and are asserted here unchanged.
    ///
    /// This layout is mirrored by the hand-written C `_Static_assert`s in
    /// `ngx_http_markdown_ffi_layout_check.h` and the regenerated cbindgen
    /// header (kept in lockstep by tasks 11.1/11.2). Do NOT weaken this
    /// test; it pins the frozen 96-byte reduced ABI.
    #[test]
    fn test_markdown_options_layout_reduced_no_selectors() {
        use std::mem::{align_of, offset_of, size_of};

        // Size shrinks 128 -> 96 (four 8-byte slots removed: two pointers
        // at 8 bytes each and two usize lengths at 8 bytes each).
        assert_eq!(size_of::<MarkdownOptions>(), 96);
        assert_eq!(align_of::<MarkdownOptions>(), 8);

        // Head fields (unchanged by the selector removal).
        ...
        // Retained tail fields shift down by 32 bytes (four removed slots).
        // `prune_noise` (u32) ends at 60; `memory_budget` (u64) is
        // 8-byte aligned, so it lands at 64.
        assert_eq!(offset_of!(MarkdownOptions, memory_budget), 64);
        assert_eq!(offset_of!(MarkdownOptions, parse_timeout_ms), 72);
        assert_eq!(offset_of!(MarkdownOptions, parser_memory_budget), 80);
        assert_eq!(offset_of!(MarkdownOptions, flush_threshold), 88);
    }
    ...
    #[test]
    fn test_ffi_encoding_chain_result_layout() {
        use std::mem::{align_of, offset_of, size_of};

        assert_eq!(size_of::<FFIEncodingChainResult>(), 12);
        assert_eq!(align_of::<FFIEncodingChainResult>(), 4);

        assert_eq!(offset_of!(FFIEncodingChainResult, classification), 0);
        assert_eq!(offset_of!(FFIEncodingChainResult, layer_count), 4);
        assert_eq!(offset_of!(FFIEncodingChainResult, layers), 8);
        assert_eq!(offset_of!(FFIEncodingChainResult, identity_present), 11);
    }

    #[test]
    fn test_ffi_chain_decode_result_layout() {
        use std::mem::{align_of, offset_of, size_of};

        assert_eq!(size_of::<FFIChainDecodeResult>(), 24);
        assert_eq!(align_of::<FFIChainDecodeResult>(), 8);

        assert_eq!(offset_of!(FFIChainDecodeResult, output), 0);
        assert_eq!(offset_of!(FFIChainDecodeResult, output_len), 8);
        assert_eq!(offset_of!(FFIChainDecodeResult, error_category), 16);
    }
    ...
    fn test_ffi_base_url_input_layout() {
        ...
        /* 144 bytes on LP64: two new pointer+len pairs (x_forwarded_for,
         * x_forwarded_port) appended after the forwarded pair. */
        assert_eq!(size_of::<FFIBaseUrlInput>(), 144);
        ...
        assert_eq!(offset_of!(FFIBaseUrlInput, x_forwarded_for), 40);
        assert_eq!(offset_of!(FFIBaseUrlInput, x_forwarded_for_len), 48);
        assert_eq!(offset_of!(FFIBaseUrlInput, x_forwarded_proto), 56);
        assert_eq!(offset_of!(FFIBaseUrlInput, x_forwarded_proto_len), 64);
        assert_eq!(offset_of!(FFIBaseUrlInput, x_forwarded_host), 72);
        assert_eq!(offset_of!(FFIBaseUrlInput, x_forwarded_host_len), 80);
        assert_eq!(offset_of!(FFIBaseUrlInput, x_forwarded_port), 88);
        assert_eq!(offset_of!(FFIBaseUrlInput, x_forwarded_port_len), 96);
        assert_eq!(offset_of!(FFIBaseUrlInput, host), 104);
        assert_eq!(offset_of!(FFIBaseUrlInput, host_len), 112);
        assert_eq!(offset_of!(FFIBaseUrlInput, is_unix_socket), 120);
        assert_eq!(offset_of!(FFIBaseUrlInput, trusted_configured), 121);
        assert_eq!(offset_of!(FFIBaseUrlInput, direct_scheme), 128);
        assert_eq!(offset_of!(FFIBaseUrlInput, direct_scheme_len), 136);
    }
