/*
 * production_conversion_harness - shared NGINX/FFI stub environment
 *
 * Single definition of the stub surface shared by the production-logic
 * conversion test suites (failopen_delivery_after_downstream_test.c and
 * delivery_decision_failopen_test.c): NGINX API stubs, FFI conversion
 * stubs, allocator stubs, metric capture macros, and the production
 * implementation includes.  Each suite includes this header once and
 * adds only its own suite-specific tests.
 */
...
/* ngx_atomic_t comes from ngx_http_markdown_filter_module.h (int). */
