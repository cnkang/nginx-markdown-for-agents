 ...
 *
 * Computed from `MARKDOWN_ABI_VERSION`, struct count, and export count.
 * Detects header regeneration drift even when the numeric version is unchanged.
 * Regenerated by the handshake when header content changes.
 */
#define MARKDOWN_HEADER_HASH 15710958600856883071ull
 ...
 * newline-joined export names (40 entries as of 0.9.2).
 */
#define MARKDOWN_SYMBOL_SET_HASH 5776572673130733471

/**
 * SHA-256 truncated hash of the ABI struct layout inventory (LP64).
 *
 * Computed from sorted `struct_name:size` pairs. Detects struct size
 * changes or additions/removals even when the numeric version is unchanged.
 */
#define MARKDOWN_LAYOUT_FINGERPRINT 9438415369764923519ull
