...
- Strict RFC 7932 single-stream enforcement: trailing data after stream
  completion is rejected as `FORMAT_ERROR`; truncated streams are detected at
  EOF
...
- Complexity debt: the remaining C and Rust production/test complexity
...
- Performance evidence provenance: the canonical 0.9.1 baseline records all
  ...
  no metric was normalized to hide a performance change.
