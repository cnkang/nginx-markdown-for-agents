...
| Incremental path | Large-response path backed by the Rust incremental API. | Retired in 0.9.0; the `markdown_large_body_threshold` directive was removed in 0.9.2 — using it now reports an `unknown directive` error at `nginx -t` time. |
