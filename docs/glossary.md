...
| Incremental path | Large-response path backed by the Rust incremental API. | Retired in 0.9.0; the `markdown_large_body_threshold` directive was retired in 0.9.0 and removed in 0.9.2 — using it now reports an `unknown directive` error at `nginx -t` time. Distinct from `markdown_stream_threshold`, which was removed in 0.9.2 with no replacement: `markdown_streaming` selects the processing path from its policy and the hard compatibility gates, and no size threshold takes part in that selection. See `markdown_streaming` and `markdown_limits` for current streaming and buffering controls. |
...
| Fail-open | Error policy that serves the original HTML when conversion fails before the response commit. | Pair with `markdown_error_policy pass`. After Markdown bytes go downstream, the original HTML cannot be served: later gzip-member failures finish the remaining Markdown safely, and only failures where safe completion is impossible truncate the stream (Post-Commit exception). |
...
| Reason code | Stable snake_case outcome string emitted by the module. | Decision-chain codes are lowercase (e.g., `not_eligible`, `converted`, `failed_open`). Streaming transitions are separate lowercase `event=` values (e.g., `streaming_convert`, `engine_streaming`) and are not reason codes. |
...
- Reason codes must match emitted log and metric labels exactly and use
  lowercase snake_case. Streaming engine transitions use separate lowercase
  `event=` values and are not reason codes.
