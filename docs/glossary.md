...
| Fail-open | Error policy that serves the original HTML when conversion fails before the response commit. | Pair with `markdown_error_policy pass`. After Markdown bytes go downstream, the original HTML cannot be served and the stream truncates (Post-Commit exception). |
