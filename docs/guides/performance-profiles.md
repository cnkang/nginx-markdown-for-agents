...
| **Streaming decompression (0.9.1)** | Not applicable (no streaming) | Not active (requires `streaming_first` profile) | Active for supported encodings (raw deflate; gzip pending) |
...
| Streaming decompression | Full-body buffering before decompression starts | Per-chunk decompression + conversion |
| Full-buffer copy reduction | Redundant linearize-copy and buffer-swap memcpy in decompression | Decompression itself + conversion |
...
- **Do expect** reduced CPU cost for full-buffer decompression of large gzip
  responses (pending benchmark validation)
...
| Streaming decompression TTFB benefit for gzip | Pending benchmark validation |
