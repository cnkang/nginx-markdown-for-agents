...
- Pre-commit conversion paths default to fail-open (`markdown_error_policy pass`) and return the original eligible HTML response. This preserves availability when conversion fails before headers are sent. After the streaming headers commit (post-commit), failures safely terminate the stream (fail-closed) because the original HTML cannot rewind. Conversion failures never expose internal details.
