...
7. `test_yaml_front_matter_requires_both_flags` - Direct-construction flag interactions. Each flag alone emits no front-matter block. The FFI layer separately promotes `extract_metadata` when `include_front_matter` is set.
