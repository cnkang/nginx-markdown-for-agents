...
7. `test_yaml_front_matter_requires_both_flags` - Direct-construction flag interactions with `ConversionOptions`. Each flag alone emits no front-matter block from a direct `Options`/`Converter` construction; the `extract_metadata` promotion when `include_front_matter` is enabled happens **only** in the FFI layer, never in direct construction.
