...
| Aspect | Detail |
|--------|--------|
| Behavior | Prepends YAML front matter to the generated Markdown (full-buffer engine only). With this directive on, `markdown_streaming auto` routes to the full-buffer engine, and `markdown_streaming force` fails `nginx -t` with a configuration error |
