...
Production-examples smoke is **0.9.0 blocking**. Soft gates emit a summary/
artifact and must not silently skip; if a soft toolchain is genuinely absent the
gate records the skip reason rather than weakening the check. Gate validators
must run in a clean checkout and must not depend on untracked local working
directories.
