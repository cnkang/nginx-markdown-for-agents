...
| Diagnostics | `markdown_diagnostics` | H/S/L | off; access control delegated to native NGINX allow/deny directives in the location block. The handler is attached to the configured location. | `ngx_http_markdown_diagnostics_handler`; diagnostics production/access/output tests and `tests/e2e/diagnostics_endpoint_test.sh` |
