...
| Authentication | `markdown_auth_policy`, `markdown_auth_cookies` | H/S/L | deny; built-in cookie patterns `session*`, `auth*`, `PHPSESSID`, `wordpress_logged_in_*` (in addition to Authorization header detection). An explicit `markdown_auth_cookies` **replaces** the built-in patterns. | auth decision paths; `auth_production_test.c` |
