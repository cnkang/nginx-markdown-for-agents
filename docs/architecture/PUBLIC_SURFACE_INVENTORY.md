...
| Authentication | `markdown_auth_policy`, `markdown_auth_cookies` | H/S/L | allow; built-in cookie patterns `session*`, `auth*`, `PHPSESSID`, `wordpress_logged_in_*` (in addition to Authorization header detection). An explicit `markdown_auth_cookies` **replaces** the built-in patterns. | auth decision paths; `auth_production_test.c` |
...
streaming, and encoding/hash helpers, so the registry includes them. The
incremental helpers stay retained internal legacy ABI symbols, not production
consumers (see below).
