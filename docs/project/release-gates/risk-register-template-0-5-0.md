...
| R3 | Post-commit failure handling: streaming path encounters error after partial output sent | Medium | High | On post-commit failure the stream is **terminated**: the module stops sending the converted representation and truncates the response at the commit boundary. Operators observe termination via the failure-outcome series of `nginx_markdown_requests_total{outcome=...,stage=...,reason=...}` and the delivery counter `nginx_markdown_conversion_deliveries_total` (fields registered at 0.5.0). The dedicated `streaming_mid_flight_error` reason code was registered later (0.9.0, discriminant 24); a 0.5.0-era register must not cite it as an existing field. Required test evidence covers a mid-flight error under real downstream backpressure (termination behavior, client-visible truncation, reason code, and operator metric deltas) |
