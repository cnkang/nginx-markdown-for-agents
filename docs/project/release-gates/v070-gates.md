...
INDEPENDENT of the Gate 3.6 version-switch test: it must come from a separate
...
| Metrics v1 registry and renderer contract (the 0.7.0-era metric rows above are historical) | metrics registry validator | `validate_metrics_registry.py` via `validate_schema_drift.py` | Current schema/release gates |
| Canonical reason registry and generated Rust/C projections (the 0.7.0-era uppercase rows are historical) | reason-code generation gate | `make reason-codegen-check` | Current release gates |
...
## Partial Automated Validation

The commands below provide partial automated validation for Gates 1, 2, 5,
and 6. They omit Gate 3's package-build, signature, checksum, install, and
upgrade/rollback checks, as well as Gate 4's promoted-cluster smoke and F5
evidence checks. Those checks require the separate procedures and evidence
validators documented above.
