#!/usr/bin/env python3
"""Extract a C function definition by name using simple brace matching.

Intended for test harnesses that compile an extracted function from a production
source file without depending on brittle line-number or grep/sed pipelines.
"""

from __future__ import annotations

import argparse
import sys
from pathlib import Path


def resolve_source_path(source: str, workspace_root: Path) -> Path | None:
    source_arg = Path(source)
    try:
        if source_arg.is_absolute():
            source_path = source_arg.resolve(strict=True)
        else:
            source_path = (workspace_root / source_arg).resolve(strict=True)
    except FileNotFoundError:
        print(f"source file not found: {source}", file=sys.stderr)
        return None

    try:
        source_path.relative_to(workspace_root)
    except ValueError:
        print(
            f"source path escapes workspace root: {source} (root: {workspace_root})",
            file=sys.stderr,
        )
        return None

    if not source_path.is_file():
        print(f"source path is not a file: {source}", file=sys.stderr)
        return None

    return source_path


def build_signature_needle(function_name: str, signature_prefix: str) -> str:
    target_line = f"{function_name}("
    if signature_prefix:
        return f"{signature_prefix}\n{target_line}"
    return target_line


def find_function_slice(src: str, function_name: str, needle: str) -> tuple[int, int] | None:
    start = src.find(needle)
    if start == -1:
        print(f"failed to find function signature for {function_name}", file=sys.stderr)
        return None

    brace_start = src.find("{", start)
    if brace_start == -1:
        print(f"failed to find function body start for {function_name}", file=sys.stderr)
        return None

    depth = 0
    for i, ch in enumerate(src[brace_start:], start=brace_start):
        if ch == "{":
            depth += 1
        elif ch == "}":
            depth -= 1
            if depth == 0:
                return start, i + 1

    print(f"failed to find function body end for {function_name}", file=sys.stderr)
    return None


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("source", help="Path to C source file")
    parser.add_argument("--function-name", required=True, help="Function name to extract")
    parser.add_argument(
        "--signature-prefix",
        default="",
        help="Exact prefix line immediately before the function name (for example 'static ngx_int_t')",
    )
    return parser.parse_args()


def main() -> int:
    args = parse_args()
    workspace_root = Path.cwd().resolve()
    source_path = resolve_source_path(args.source, workspace_root)
    if source_path is None:
        return 1

    src = source_path.read_text(encoding="utf-8")
    needle = build_signature_needle(args.function_name, args.signature_prefix)
    slice_bounds = find_function_slice(src, args.function_name, needle)
    if slice_bounds is None:
        return 1
    start, end = slice_bounds

    sys.stdout.write(src[start:end] + "\n")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
