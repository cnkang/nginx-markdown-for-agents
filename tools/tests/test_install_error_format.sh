...
_SHIM_DIRS=()
# _cleanup_shim_dirs removes temporary shim directories recorded in _SHIM_DIRS, ignoring any removal errors.
_cleanup_shim_dirs() {
...
# fail increments FAIL_COUNT, prints a red "FAIL" line with the provided message to stderr, and, if a detail is supplied, prints an indented "Detail: <detail>" line.
fail() {
  local msg="$1"
  local detail="${2:-}"
...
# assert_has_error_format checks whether the given stderr text contains a line that matches "[ERROR] <category>: <message>" where <category> is lowercase letters and underscores and <message> is non-empty.
...
# assert_has_suggest_format checks whether the given stderr text contains at least one line starting with "[SUGGEST]" followed by a suggestion.
...
# build_path_hiding creates a temporary shim directory that hides the specified binary from PATH, echoes the modified PATH with the shim directory placed first to stdout, prints the shim directory path to stderr, and records the shim in _SHIM_DIRS for later cleanup.
build_path_hiding() {
  local binary_name="$1"
  local shim_dir
  shim_dir="$(mktemp -d)"
...
run_hiding_python3_with_fake_nginx() {
  ...
  local restricted_path
  restricted_path="$(build_path_hiding python3 2>"$shim_info")"
  local shim_dir
  shim_dir="$(cat "$shim_info")"
  rm -f "$shim_info"
...
# -----------------------------------------------------------------------
# Test 4: hiding python3 still produces exactly one JSON error document
# -----------------------------------------------------------------------
...
# -----------------------------------------------------------------------
# Test 5: Static analysis — every exit 1 preceded by emit_error/die_with_error
# -----------------------------------------------------------------------
echo "Test 5: Static analysis — exit 1 paths use structured error helpers" >&2
...
if command -v python3 > /dev/null 2>&1; then
  ANALYZE_SCRIPT="$(dirname "${BASH_SOURCE[0]}")/analyze_exit1_paths.py"
  STATIC_RESULT="$(python3 "$ANALYZE_SCRIPT" "$INSTALL_SCRIPT")"
