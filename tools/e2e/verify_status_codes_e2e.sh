#!/usr/bin/env bash
# verify_status_codes_e2e.sh — Thin wrapper for the status-codes E2E scenario.
#
# Delegates execution to the Rust e2e-harness binary which validates
# upstream HTTP status-code passthrough behaviour (200, 301, 404, 500,
# 206, etc.) to ensure the module does not interfere with non-200
# responses that should not be converted.
#
...
#
# Usage:
#   tools/e2e/verify_status_codes_e2e.sh [--keep-artifacts] [--port PORT] [--nginx-bin PATH]
#
# Exit behaviour:
