...
if run_case caller-path-selection 47 '
set -e
DYNCONF_FILE="$2"
source "$1"
TEST_TMPDIR="$3"
...
if run_case default-path-selection 46 '
set -e
source "$1"
TEST_TMPDIR="$3"
...
if run_case external-without-opt-in 44 '
set -e
source "$1"
TEST_TMPDIR="$3"
OWN_TEST_TMPDIR=1
...
if run_case same-second-mtime 0 '
set -e
source "$1"
...
: > "$KILL_LOG"
if run_pid_case no-pid-no-kill 0 '
...
# signal either one.
: > "$KILL_LOG"
if run_pid_case two-masters-no-signal 0 '
unset NGINX_PID HARNESS_NGINX_PID NGINX_PID_FILE
set +e
resolve_nginx_pid >/dev/null
rc=$?
...
: > "$KILL_LOG"
if run_pid_case explicit-pid-resolved 0 '
...
: > "$KILL_LOG"
if run_pid_case explicit-pid-reload 0 '
export NGINX_PID=77777
export STUB_PID=77777
...
if run_pid_case invalid-pid-non-numeric 0 '
...
if run_pid_case invalid-pid-zero 0 '
export NGINX_PID=0
set +e
resolve_nginx_pid >/dev/null
rc=$?
...
if run_pid_case invalid-pid-self 0 '
export NGINX_PID=$$
set +e
resolve_nginx_pid >/dev/null
rc=$?
...
: > "$KILL_LOG"
if run_pid_case invalid-pid-send-reload 0 '
export NGINX_PID="abc; rm -rf /"
set +e
...
if run_pid_case missing-pid-send-reload 0 '
...
if run_pid_case prefix-bypass-rejected 0 '
export NGINX_PID=77777
...
: > "$KILL_LOG"
if run_pid_case no-pid-send-reload 0 '
...
: > "$KILL_LOG"
if run_pid_case pid-file-missing-send-reload 0 '
...
: > "$KILL_LOG"
if run_pid_case pid-file-nonregular-send-reload 0 '
...
: > "$KILL_LOG"
if run_pid_case pid-file-canonicalize-failure 0 '
...
if run_pid_case pid-file-missing-resolve 0 '
export NGINX_PID_FILE="'"$pid_file_missing"'"
set +e
resolve_nginx_pid >/dev/null
rc=$?
...
printf "99999\n" > "$pid_file_unreadable"
: > "$KILL_LOG"
if run_pid_case pid-file-unreadable 0 '
...
printf "12345\n67890\n" > "$pid_file_multi"
if run_pid_case pid-file-multi-line 0 '
...
printf "not-a-pid\n" > "$pid_file_nonnum"
if run_pid_case pid-file-non-numeric 0 '
...
printf " 99999 \n" > "$pid_file_space"
: > "$KILL_LOG"
if run_pid_case pid-file-surrounding-spaces 0 '
...
for pid_file_case in internal-space internal-tab extra-token; do
    ...
    : > "$KILL_LOG"
    if run_pid_case "pid-file-$pid_file_case" 0 '
...
: > "$KILL_LOG"
if run_pid_case pid-file-symlink-send-reload 0 '
...
printf "99999\n" > "$pid_file_valid"
if run_pid_case pid-file-valid-resolve 0 '
...
: > "$KILL_LOG"
if run_pid_case pid-file-valid-reload 0 '
export NGINX_PID_FILE="'"$pid_file_valid"'"
export STUB_PID=99999
...
: > "$KILL_LOG"
if run_pid_case explicit-source-priority 0 '
export NGINX_PID=77777
export NGINX_PID_FILE="'"$pid_file_valid"'"
...
: > "$KILL_LOG"
if run_pid_case pid-file-source-priority 0 '
unset NGINX_PID
export NGINX_PID_FILE="'"$pid_file_valid"'"
export HARNESS_NGINX_PID=88888
export STUB_PID=99999
...
: > "$KILL_LOG"
if run_pid_case master-correct-prefix 0 '
export NGINX_PID=77777
export NGINX_PID_PREFIX=/test/nginx
...
: > "$KILL_LOG"
if run_pid_case master-wrong-prefix 0 '
export NGINX_PID=77777
...
: > "$KILL_LOG"
if run_pid_case remote-url-no-kill 0 '
...
: > "$KILL_LOG"
if run_pid_case harness-owned-reload 0 '
export HARNESS_NGINX_PID=88888
...
if run_case cleanup-restore-fail-rc0 70 '
set -e
source "$1"
TEST_TMPDIR="$3"
OWN_TEST_TMPDIR=1
...
if run_case cleanup-restore-fail-rc41 41 '
set -e
source "$1"
TEST_TMPDIR="$3"
OWN_TEST_TMPDIR=1
...
if run_case cleanup-rmdir-fail-rc0 70 '
set -e
source "$1"
TEST_TMPDIR="$3"
OWN_TEST_TMPDIR=1
