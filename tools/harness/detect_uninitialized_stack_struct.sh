...
while IFS= read -r -d '' file; do
    ...
    for type in "${WHOLE_INIT_TYPES[@]}"; do
        ...
        while IFS=: read -r line_num content; do
            ...
            # Whole-init present?
            if echo "$tail_code" | grep -qE "memset[[:space:]]*\(&${var_name}([^[:alnum:]_]|$)|memzero[[:space:]]*\(&${var_name}([^[:alnum:]_]|$)|(^|[^[:alnum:]_])${var_name}[[:space:]]*=[[:space:]]*\{|(^|[^[:alnum:]_])${var_name}[[:space:]]*=[[:space:]]*0;|_init[[:space:]]*\(&${var_name}([^[:alnum:]_]|$)|init_[a-z_]+[[:space:]]*\([^;]*&${var_name}([^[:alnum:]_]|$)"; then
                continue
            fi
