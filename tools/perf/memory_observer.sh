...
# log writes its arguments to stderr and returns success.
...
# now_ms returns the current Unix epoch time in milliseconds as an integer.
now_ms() {
  if [[ "$OS" == "Darwin" ]]; then
...
# _capture_process_fingerprint returns the full launch timestamp for the given PID (used as a process fingerprint); on failure it echoes an empty string.
_capture_process_fingerprint() {
  ...
  if [[ "$OS" == "Linux" ]]; then
    ...
    # spaces, so we strip everything up to the closing ')' first, then
    # pick the correct field from the remainder (starttime is field 20
    # of the post-comm tail).
    sed 's/^[^)]*) //' "/proc/$pid/stat" 2>/dev/null | awk '{print $20}' || echo ""
  else
...
# process_alive checks whether the target PID is still running and, if PROCESS_FINGERPRINT is set, verifies the PID has not been reused by comparing the current fingerprint to the stored one. It returns success (0) when the process exists and the fingerprint matches (or no fingerprint is set), and returns non-zero if the PID does not exist or the fingerprint differs. It logs a warning when a fingerprint mismatch (PID reuse) is detected.
process_alive() {
...
# interval_to_sleep converts a millisecond interval to a fractional-second string formatted to three decimal places suitable for sleep.
interval_to_sleep() {
  local ms="$1"
