...
# assert_json checks that a file exists and contains valid JSON, prints a PASS/FAIL message, and updates the global PASS/FAIL counters.
# Arguments: the file path to validate and a short label used in log messages. Returns non-zero on failure.
assert_json() {
  local file="$1"
  local label="$2"
  if [[ ! -f "$file" ]]; then
