...
@pytest.mark.skipif(
    ...
    reason="requirements-perf.txt is required for the live benchmark process",
)
...
class TestNginxConfigGeneration:
    ...
    def test_preflight_requires_rss_commands(self):
        """RSS evidence prerequisites must fail before benchmark execution."""
        script_content = BENCHMARK_SCRIPT.read_text(encoding="utf-8")

        assert "required_commands=(" in script_content
        assert "  ps" in script_content
        assert 'command -v "$required_command"' in script_content
        assert "ps is required for RSS evidence" in script_content
