...
def test_nightly_reuses_the_candidate_baseline_measurement() -> None:
    """The canonical 092 gate must reuse its validated candidate measurement."""
    block = _module_baseline_job(_nightly_perf_text())
    evidence_gate = block.index("make perf-evidence-check")
    release_gates = block.index("make release-gates-check-092-canonical")
    report = (
    ...
    assert evidence_report in block
...
def test_release_source_provenance_is_built_and_required() -> None:
    """The release run must build source provenance from the released commit.

    A digest of an archive generated by the hosting service anchors nothing and
    cannot be recorded before the tag exists.  The workflow therefore builds the
    bundle itself from the released commit, records its digest, publishes the
    bundle, and fails the release when that digest is missing.  The registry under
    packaging/ stays as an audit record: when it holds an entry the run compares
    it with the digest it built and fails on disagreement.
    """
    repo_root = Path(__file__).resolve().parents[3]
    workflow = (repo_root / ".github" / "workflows" / "release-packages.yml").read_text(
        encoding="utf-8")
    assert "git archive --format=tar" in workflow, (
        "the run must build the source bundle from the repository"
    )
    assert "gzip -n" in workflow, "the bundle must be byte-reproducible"
    assert '"${GH_SHA}" | gzip -n' in workflow, (
        "the bundle must come from the exact released commit"
    )
    assert "--source-sha" in workflow
    assert "packaging/source-archive-digests.sha256" in workflow, (
        "the registry stays as an audit record"
    )
    assert 'if [[ -n "${RECORDED_SHA}" && "${RECORDED_SHA}" != "${SOURCE_SHA}" ]]' in workflow, (
        "a registry entry must agree with the digest this run built"
    )
    assert "::error title=Source bundle missing::" in workflow, (
        "a tag release without a digest must fail rather than warn"
    )
    assert "Source archive digest not recorded" not in workflow, (
        "the old warn-and-continue path must not come back"
    )
    assert 'sha256sum "${bundle}" | awk' in workflow, (
        "the digest must be computed from the bundle this run built"
    )
    # The abandoned design downloaded the tag archive and hashed the response.
    assert "archive/refs/tags" not in workflow, (
        "the release run must not derive the source digest from a remote archive"
    )
