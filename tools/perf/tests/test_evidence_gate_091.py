...
def test_module_baseline_contains_measured_critical_scenarios():
    ...
    nginx_version=unknown) because it was generated before the path-coverage
    and RSS sampling fixes.  The evidence gate correctly rejects it as
    MISSING_EVIDENCE.  The baseline must be regenerated by running a real
    benchmark after all benchmark runtime fixes are deployed.
    """
    baseline_path = (
        Path(__file__).resolve().parents[3]
        / "perf"
