...
def parse_install_script(path: Path) -> dict:
    ...
    Returns:
        dict: Mapping with the following keys:
            ...
            asset_name_template (str): ASSET_NAME pattern normalized to Python-style placeholders `{nginx}`, `{os_type}`, and `{arch}`.
...
def _collect_architecture_mismatch_errors(install_info, matrix_archs, errors):
    ...
    Parameters:
        ...
        errors (list[str]): Mutable list that will receive a single combined error message describing architectures only in the script and architectures only in the matrix.
    """
