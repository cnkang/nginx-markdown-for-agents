...
fn bridge_to_native_build(base_dir: &Path) -> Result<BootstrapResult> {
    ...
    if !output.status.success() {
        ...
        if cfg!(target_os = "macos") {
            ...
            msg.push_str("The native build requires OpenSSL, PCRE2, and zlib.\n");
            ...
            msg.push_str("`brew --prefix` and exports CPPFLAGS/LDFLAGS.\n");
            msg.push_str("If linking still fails, pass paths explicitly:\n");
            msg.push_str("  export CPPFLAGS=\"-I$(brew --prefix openssl@3)/include -I$(brew --prefix pcre2)/include -I$(brew --prefix zlib)/include\"\n");
            msg.push_str("  export LDFLAGS=\"-L$(brew --prefix openssl@3)/lib -L$(brew --prefix pcre2)/lib -L$(brew --prefix zlib)/lib\"\n\n");
