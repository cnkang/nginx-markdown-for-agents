...
#
# REQUIREMENTS:
#   - Ingress/Controller custom image build
...
test_module_file_exists() {
    ...
    check_output="$(docker run --rm --entrypoint ls "$IMAGE_TAG" \
        -la "$expected_path" 2>&1)" || check_rc=$?
    ...
    local file_size
    file_size="$(docker run --rm --entrypoint stat "$IMAGE_TAG" \
        -c '%s' "$expected_path" 2>/dev/null || \
        docker run --rm --entrypoint stat "$IMAGE_TAG" \
        -f '%z' "$expected_path" 2>/dev/null)" || true
