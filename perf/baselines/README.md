...
- `module-baseline-091-raw-probes/` - retained response probe artifacts
  ...
  validates all eight scenario triplets before upload. The `.body` and
  `.headers` sidecar files referenced by the raw report are committed
  alongside it (identical content to the 0.9.2 raw-probe set, verified by
  SHA-256).
