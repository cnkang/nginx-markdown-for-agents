...
- `module-baseline-092.json` - Linux x86_64 module-level 0.9.2 evidence
  baseline. The checked-in copy is currently ineligible for the release
  gates (audit context only); an eligible baseline with a CI-bound
  provenance is produced by the nightly workflow and the release run
  (`make release-gates-check-092` uses whichever baseline the gate policy
  selects)
